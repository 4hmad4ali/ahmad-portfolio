...
export default function ProjectGallery({ images, projectTitle }: { images: GalleryImage[]; projectTitle: string }) {
  ...
  return (
    <figure aria-label={`${projectTitle} interface gallery`}>
      <div className="overflow-hidden rounded-3xl border border-foreground/15 bg-foreground/[0.03] shadow-2xl shadow-foreground/5">
        <div className="flex items-center justify-between border-b border-foreground/10 px-4 py-3 md:px-6">
          ...
          <span className="max-w-[60%] truncate font-ibmplexmono text-[11px] text-foreground/55">{activeImage.label}</span>
          <a
      ...
      {images.length > 1 && (
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4" role="group" aria-label="Choose a project screenshot">
            ...
            return (
              ...
              >
                <span className="relative block aspect-video overflow-hidden rounded-xl bg-foreground/5">
                  <Image src={image.src} alt="" fill sizes="(max-width: 640px) 45vw, 260px" className="object-cover transition-transform duration-500 motion-safe:group-hover:scale-105" />
                </span>
                <span className={`mt-2 block truncate px-1 text-xs ${isActive ? "font-medium text-foreground" : "text-foreground/60"}`}>{image.label}</span>
              </motion.button>
            );
          })}
        </div>
      ...
      <figcaption className="mt-4 flex items-center justify-between gap-4 text-xs text-foreground/55">
        <span>{activeImage.alt}</span>
        <span className="shrink-0 font-ibmplexmono">{String(activeIndex + 1).padStart(2, "0")} / {String(images.length).padStart(2, "0")}</span>
      </figcaption>
    </figure>
  );
}
